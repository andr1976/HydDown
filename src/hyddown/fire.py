...
- API 521 pool fire peak: elevated temperature peak pool fire
...
def jet_fire_peak_large_api521(Tvessel):
    """
    Calculate heat flux for API521 large peak jet fire scenario.
...
def pool_fire_peak_api521(Tvessel):
    """
    Calculate heat flux for API 521 peak pool fire scenario.

    Implements a peak pool fire based on API 521 parameters with elevated
    flame and radiative temperatures representing localized hot-spot
    conditions or large fully-engulfing pool fires.
    ...
    - Surface emissivity (ε_surface): 0.75
    - Convective heat transfer coefficient (h): 20 W/(m²·K)
    - Flame temperature: 1050°C (1323.15 K)
    - Radiative temperature: 1050°C (1323.15 K)
...
def sb_fire(T_vessel, fire_type):
    ...
    fire_type : str
        ...
        - "api_pool_peak": API 521 peak pool fire
