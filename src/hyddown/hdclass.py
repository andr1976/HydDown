...
class HydDown:
    ...
    def _initialize_heat_transfer_model(self):
        ...
        # Add detailed wall model parameters if thermal_conductivity AND thickness are specified
        if "thermal_conductivity" in self.input["vessel"] and "thickness" in self.input["vessel"]:
            ...
            # Tamb NOT added here - passed separately to ConvectiveHeatTransfer constructor
        ...
        if self.heat_method in ["specified_h", "detailed"]:
            ...
            # Tamb is required by ConvectiveHeatTransfer as separate parameter
        ...
        for i in tqdm(
            ...
            # where U = internal energy, Q = heat transfer rate, H = specific enthalpy
                ...
                if self.heat_method in ["specified_h", "detailed", "s-b"]:
                    result = self.heat_transfer_model.calculate(
                        i=i,
                        t=self.time_array[i],
                        ...
                        fluid=self.fluid,
                        transport_fluid=self.transport_fluid,
                        transport_fluid_wet=self.transport_fluid_wet,
                        mass_rate=self.mass_rate[i - 1],
                    )

                    # Unpack heat transfer results
                    self.Q_inner[i] = result['Q_inner']
                    self.Q_outer[i] = result['Q_outer']
                    self.Q_inner_wetted[i] = result['Q_inner_wetted']
                    self.Q_outer_wetted[i] = result['Q_outer_wetted']
                    self.q_inner[i] = result['q_inner']
                    self.q_outer[i] = result['q_outer']
                    self.q_inner_wetted[i] = result['q_inner_wetted']
                    self.q_outer_wetted[i] = result['q_outer_wetted']
                    self.h_inside[i] = result['h_inside']
                    self.h_inside_wetted[i] = result['h_inside_wetted']
                    self.T_vessel[i] = result['T_vessel']
                    self.T_vessel_wetted[i] = result['T_vessel_wetted']
                    self.T_inner_wall[i] = result['T_inner_wall']
                    self.T_outer_wall[i] = result['T_outer_wall']
                    self.T_inner_wall_wetted[i] = result['T_inner_wall_wetted']
                    self.T_outer_wall_wetted[i] = result['T_outer_wall_wetted']

                    if result['T_bonded_wall'] is not None:
                        self.T_bonded_wall[i] = result['T_bonded_wall']
                        self.T_bonded_wall_wetted[i] = result['T_bonded_wall_wetted']
