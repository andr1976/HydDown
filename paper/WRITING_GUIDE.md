...
author's established scientific voice. **Tone throughout is neutral, formal and precise, with
no colourful language, no superlatives and no rhetoric.**

> **Punctuation rule: do NOT use em-dashes anywhere** (neither the character `—` nor the LaTeX
> `---`). Use commas, parentheses or colons, or restructure the sentence. An em-dash may be used
> only for an extremely good reason where no other punctuation works, **and only with the
> author's explicit approval.** En-dashes (`--` in LaTeX) remain correct for numeric ranges
> (e.g. `200--500 s`) and name compounds (e.g. Churchill--Chu). This applies to the manuscript,
> to all supporting text, and to normal correspondence.

Style sources studied:
- Andreasen & Stegelmann (2025), *Open source pressure vessel blowdown modelling under
  partial phase equilibrium*, chemRxiv 2025-00xzc (the closest template: same topic family,
...
- **Reference style: Harvard (author-date).** In text (directly or parenthetically): single
  ...
  *"Kramer et al. (2023) have recently shown"*. Reference list alphabetical then chronological;
  same author+year distinguished by `a`, `b`, `c`. Abbreviate journal names per the LTWA.
  List format example: *"Van der Geer, J., Handgraaf, T., Lupton, R.A., 2020. The art of writing
  a scientific article. J. Sci. Commun. 163, 51-59. https://doi.org/10.1016/j.sc.2020.0037."*
  (article-number variant also given). Set in `main.tex` via the `authoryear` class option +
  `elsarticle-harv.bst` + `\citep{}` / `\citet{}`.
- **Structure:** IMRaD, i.e. Introduction, Methods (here "Model"), Results and discussion,
  Conclusions. Frontmatter: title, authors + affiliations, abstract, keywords, highlights.
- **Abstract:** single unstructured paragraph; concise; no citations or abbreviations.
- **Highlights: REQUIRED at submission.** A short set of bullet points capturing the novel
  results and any new methods; **at most 85 characters each, including spaces** (Elsevier
  standard: 3 to 5 bullets).
...
- **Keywords:** four to six, separated by `\sep`.
- **CRediT authorship contribution statement (required):** roles such as Conceptualization,
  Data curation, Formal analysis, Funding acquisition, Investigation, Methodology, etc.
...
- **Data statement / availability + data linking:** state where code and data live (the HydDown
...
- **Neutral, formal, impersonal.** State facts and observations plainly and let the numbers
  carry the claim. No adjectives of enthusiasm.
...
- **Passive / impersonal for everything else:** descriptions, method steps and especially
  results, e.g. *"comparison … is made"*, *"It is also noted that …"*, *"the pressure … is
  predicted very well"*, *"Data have been sourced from ref. …"*.
...
- Quantify discrepancies with magnitude and location, e.g. *"a slight over-prediction, most
...
- Mechanistic explanations are offered tentatively, e.g. *"It appears that slight differences in
  the heat-transfer modelling may be one of the reasons …"*.
...
2. Literature review: summarise each key prior model/experiment in 2 to 4 sentences (its
   approach and findings) and note its limitation or gap, e.g. *"Witkowski and Majkut (Year)
...
processes), then one numbered subsection per sub-model: thermodynamics/phase equilibrium, mass
and energy balance, discharge, heat transfer, dry-ice/below-triple, and code implementation.
State assumptions explicitly; number all equations; cite the open-source packages used (CoolProp,
fluids, and thermopack for the offline solid table).
...
- List the cases/experiments with data provenance (*"Data have been sourced from ref. …"*) and a
  summary table with a detailed caption (initial conditions, geometry, Cd, assumptions).
...
**Conclusions:** a concise restatement of what was done, the validated capability, the key
quantitative findings, and the honest limitations (e.g. the 0-D stratification limitation).

## 5. Citations, figures, tables, reproducibility

- Weave citations into the sentence, e.g. *"a much more recent published model by Park et al.
  (Year) shares many similarities …"* / *"For a comprehensive review … the recent study of
...
- Emphasise openness and reproducibility: the tool is open source and free; the datasets are
...
3. Model: thermodynamics and the solid-CO₂ lever; discharge (HEM + non-equilibrium); in-vessel
   dry ice (two-zone plateau/descent); heat and mass transfer.
