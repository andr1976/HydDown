...
th=np.load(os.path.join(H,"t72therm_out.npz")); lu=np.load(os.path.join(H,"test72_out.npz"))
...
ax[0,1].axhline(11.7,color=grey,ls=":",lw=1.2,label="measured retained 11.7 kg")
...
ax[1,1].axhline(11.7,color=grey,ls=":",lw=1.2,label="measured 11.7 kg")
...
fig.suptitle("Munkejord Test 72 (6.5 mm, no riser) - thermesh vs lumped wall vs 1 Hz data",fontsize=11)
fig.tight_layout(rect=[0,0,1,0.97]);fig.savefig(os.path.join(H,"test72_thermesh_overlay.pdf"));fig.savefig(os.path.join(H,"test72_thermesh_overlay.png"));print("wrote test72_thermesh_overlay",flush=True)
