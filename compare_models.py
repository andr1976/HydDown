"""Compare 1D vs 0D heat transfer models for rupture case."""
import matplotlib.pyplot as plt
import numpy as np
import yaml
from hyddown import HydDown

# Run 0D model (lumped capacitance)
print("Running 0D model (rupture.yml)...")
with open("src/hyddown/examples/rupture.yml") as f:
    input_0d = yaml.safe_load(f)
hdown_0d = HydDown(input_0d)
hdown_0d.run()

# Run 1D model (detailed heat transfer)
...
# Plot 3: Wall temperature comparison - MAIN COMPARISON
ax = axes[1, 0]
...
ax.set_title('Wall Temperature Comparison (Unwetted)')
ax.legend()
ax.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig('comparison_0d_vs_1d.png', dpi=150)
print("\nPlot saved as 'comparison_0d_vs_1d.png'")
...
# Check wetted wall issue
print(f"\n  Wetted Wall Temperatures (should not exist for gas-only):")
print(f"    1D model wetted inner: {hdown_1d.T_inner_wall_wetted[-1]-273.15:.2f} °C")
print(f"    1D model wetted outer: {hdown_1d.T_outer_wall_wetted[-1]-273.15:.2f} °C")
print(f"    Liquid level: {hdown_1d.liquid_level[-1]:.6f} m")
